...
public class ComptabiliteManagerImpl extends AbstractBusinessManager implements ComptabiliteManager {
    ...
    public synchronized void addReference(EcritureComptable pEcritureComptable) throws NotFoundException {
        ...
        int annee = pEcritureComptable.getDate().toInstant().atZone(zoneId).getYear();
    ...
    /**
     * Vérifie la Règle de compta N°2 : Pour qu'une écriture comptable soit valide, elle doit être équilibrée
     *
     * @param pEcritureComptable
     * @throws Exception
     */
    ...
    public void checkEcritureComptableUnitRG2 (EcritureComptable pEcritureComptable) throws FunctionalException {
        ...
        System.out.println(pEcritureComptable.getTotalCredit() + " " + pEcritureComptable.getTotalDebit());
    ...
    /**
     * Vérifie la Règle de compta N°3 : Une écriture comptable doit avoir au moins 2 lignes d'écriture (1 au débit, 1 au crédit)
     *
     * @param pEcritureComptable
     * @throws Exception
     */
    ...
    public void checkEcritureComptableUnitRG5 (EcritureComptable pEcritureComptable) throws FunctionalException{
        ...
        int annee = pEcritureComptable.getDate().toInstant().atZone(ZoneId.of("Europe/Paris")).getYear();
        int refLenght = pEcritureComptable.getReference().length();
        String reference = pEcritureComptable.getJournal().getCode()+"-"+annee+"/";
        if(!pEcritureComptable.getReference().contains(reference) || refLenght != 13) throw new FunctionalException(
                "Erreur de format ou d'écriture de référence."
        );
    }
